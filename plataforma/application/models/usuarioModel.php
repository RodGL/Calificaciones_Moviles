...
//Carga el acceso configurada en application|config|database.php
    ...
    public function query($idusuario = '') {
        ...
        if (empty($idusuario)) { //verifica si hay una usuario en específico
            ...
            $qry = $this->db->get('usuario', array('iduser' => $idusuario));
        }
        $data = array();
...
//Verifica que no vaya vacío.
        if (isset($idusuario)) {
            $stmtDelete = "UPDATE `usuario` SET `estatus`= 2  where iduser = " . $idusuario;
...
//Verifica que no vaya vacío.
        if (isset($idusuario)) {
            $stmtDelete = "UPDATE `usuario` SET `estatus`= 1  where iduser = " . $idusuario;
            $this->db->query($stmtDelete);
        }
    }
